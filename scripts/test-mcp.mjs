...
console.log('── 2. actor.set-move pushes intent into collectInput ──');
frame();
assert.equal(input.moveY, 0, 'input.moveY starts at 0');
await registry.call('actor.set-move', { moveY: 1, run: true, holdMs: 2000 });
frame();
...
console.log('── 3. actor.stop clears intent ──');
await registry.call('actor.stop', {});
frame();
assert.equal(input.moveY, 0, 'input.moveY zeroed by actor.stop');
assert.equal(input.run, false, 'input.run cleared by actor.stop');
console.log(`  input.moveY=${input.moveY} input.run=${input.run}`);

console.log('── 4. actor.turn-by adds turnDelta once ──');
await registry.call('actor.turn-by', { radians: 0.5 });
frame();
...
console.log('── 5. actor.fire / stop-fire toggle input.fireHeld ──');
input.fireHeld = false;
await registry.call('actor.fire', { durationMs: 0 });
assert.equal(input.fireHeld, true, 'fire held');
await registry.call('actor.stop-fire', {});
assert.equal(input.fireHeld, false, 'fire released');
console.log('  fireHeld toggled correctly.');

console.log('── 6. actor.fire durationMs auto-releases ──');
await registry.call('actor.fire', { durationMs: 80 });
assert.equal(input.fireHeld, true, 'fire held');
await new Promise((r) => setTimeout(r, 160));
assert.equal(input.fireHeld, false, 'fire auto-released after duration');
console.log('  fireHeld auto-released after 80ms.');

console.log('── 7. actor.switch-weapon does not throw ──');
await registry.call('actor.switch-weapon', { slot: 3 });
await registry.call('actor.switch-weapon', { slot: 1 });
console.log('  switch-weapon did not throw.');

console.log('── 8. actor.use invokes pressUse → pendingFlags.use = true ──');
await registry.call('actor.use', {});
...
console.log('── 11. actor.possess with bad thing id returns ok:false ──');
const bad = await registry.call('actor.possess', { targetId: 'thing:99999' });
